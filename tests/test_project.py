...
class TestProject(unittest.TestCase):
    ...
    # Verifica che la classe Project accetti correttamente le classi delle strutture e che inizializzi gli alberi e le fabbriche di nodi corrispondenti.
    ...
    # Verifica che la funzione create_node della classe Project restituisca un nodo del tipo corretto in base alla struttura attualmente selezionata.
    ...
    # Verifica che la funzione set_structure della classe Project aggiorni correttamente la struttura, l'albero e la fabbrica di nodi quando viene chiamata con un nuovo tipo di struttura.
    ...
    # Verifica che la funzione set_n della classe Project aggiorni correttamente il valore di n quando viene chiamata con un nuovo valore.
    def test_set_n_updates_n(self):
        project = Project("BST", 10)
        project.set_n(999)
        self.assertEqual(project.n, 999)

    # Verifica che la classe Project sollevi un'eccezione ValueError quando viene passato un nome di struttura non valido.
    def test_invalid_structure_raises(self):
        with self.assertRaises(ValueError):
            Project("NOT_A_TREE", 10)

    # Verifica che la classe Project sollevi un'eccezione ValueError quando viene passato un valore non valido per n.
